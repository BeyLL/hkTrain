    ...
    try {
        ...
        permiss.checkMobile2Token(user,token,function (data) {
            if(data){
                ...
                var createtime = query.createtime ||-1;
                var is_push = query.is_push || 0;
                if(plan_point==-1||creater==-1|| title==-1||createtime==-1 ||unit_id==-1){
                ...
                getData.insert_batch("t_pushPlan",fileArr,dataArr,null,null,function (err,data) {
                    if(err==0){
                        if(plan_point==1){
                            var bbq = "指示";
    ...
    try{
        ...
        permiss.checkMobile2Token(user,token,function (data) {
            if(data){
                ...
                var creater = body.creater;
                var title =body.title;
                var info =body.info;
                var createtime =body.createtime;
                var plan_point =body.plan_point;
                var is_push =body.is_push;
                if(id==-1){
                    res.json({"code":101,"msg":"参数错误"});
                    return;
                }
