                    ...
                    if(id==-1){
                        ...
                        getData.select_data("t_user","count(0) as total","id",id,null,null,function (err,data) {
                            if(err){
                                ...
                                if(data[0].total==0){
                                    res.end(JSON.stringify({"code": 302, "msg": "该用户不存在!"}));
                                }else{
...
exports.delete_user = function (req,res) {
    ...
    try{
        ...
        var user = body.user ||-1;
        var token =body.token||-1;
        if(id==-1 || user ==-1 || token==-1){
            res.json({ "code": 100, "msg":"参数不全" });
            return;
        }
        permiss.checkMobile2Token(user,token,function (datas) {
            if(datas){
                getData.delete_data("t_user",null,"id",id,function (err,data) {
                    if(err){
                        res.json({ "code": 300, "msg": err.message});
                        return;
                    }else{
                        log.insertLog(user,"人员管理","删除学员");
